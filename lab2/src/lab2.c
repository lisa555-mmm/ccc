...
/*
 * execute_iptables() - 執行 iptables 命令
 */
void execute_iptables(const char* command);
void execute_iptables(const char* command) {
    int result = system(command);
    if (result != 0) {
        fprintf(stderr, "[ERROR] Failed to execute iptables command: %s\n", command);
    }
    else {
        printf("[INFO] Executed iptables command: %s\n", command);
    }
}
...
void detect_arp_spoof(uint8_t* ip, uint8_t* mac) {
    ...
    if (expected_mac != NULL && memcmp(expected_mac, mac, 6) != 0) {
        ...
        arptable_add(ip, mac); // 新的 IP-MAC 配對應該被添加到 ARP 表
    }
}


/*
 * clear_iptables_rules() - 清理舊的 iptables 規則
 */
